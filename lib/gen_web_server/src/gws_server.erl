...
handle_complete_packet(#state{body = Body} = State) when State#state.connection == persistent ->
	...
	ok ->
        ...
        proxy ->
	    gen_tcp:send(State#state.socket, Reply),
            %%call the callback again with the socket information
            {_Disposition, _Reply, UserState} = callback(
		NewState#state{request_line = {proxy_request, State#state.socket}}),
            terminate(NewState#state{user_state = UserState, request_line = undefined});
	stop  ->
	    gen_tcp:send(State#state.socket, Reply),
	    terminate(NewState)
    end;
